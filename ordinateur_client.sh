...
user="wilder"
IP="172.16.10.20"
machine="$user@$IP" # Combinaison pour SSH
MDP="Azerty1*"
function connexion {
...
function actions_ordinateur {
    ...
    11)
        ...
        if dpkg -l | grep $soft >/dev/null; then
            ...
            connexion sudo apt update && sudo apt install $soft -y
            afficher_succes
        fi
        ;;
    12)
        ...
        if dpkg -l | grep $soft >/dev/null; then
            connexion sudo apt remove $soft -y
            if dpkg -l | grep $soft >/dev/null; then
    ...
    13) if confirmation; then
        ...
        connexion cd $path && chmod 755 $name && ./$name
    fi ;;
    R)
        choix_ordinateur
...
function informations_utilisateur {
    ...
    if [ $nbr_info -le 1 ]; then
        ...
        5) if confirmation; then
            ...
            if [ -f $file ]; then
                connexion ls -l $file | awk '{print $1" "$3" "$4}'
